...
App launches
AsyncStorage checks for saved JWT token
User is redirected:
Logged in → Theatre list
...
Seat map loads
User selects seats
Reservation request sent to backend
Backend:
Verifies JWT
Validates seat availability
Creates reservation
Stores reserved seats
Reservation appears in "My Bookings"
Installation
Prerequisites
...
The frontend automatically detects the backend IP address during Expo development.

Auto Detection

The app reads the Metro host IP and automatically connects to:

http://<your-ip>:5000
Manual Override

You can manually define the backend URL using:
...
Frontend lint currently fails because of one unescaped apostrophe in:

frontend/app/login.tsx

Fixing that resolves the lint warning.
